...
condition <- expand.grid(test_length, item_character, change_theta, CO_effect, cor_subdim)
colnames(condition) <- c("test_length", "item_character", "change_in_theta","carry-over", "eff_size_cor_sub_attr")
...
while(num_test <= dim(condition)[1]){
  ...
  if(condition[num_test, 5] == "small"){
    cov_mat <- cov_mat_sig1[[1]]
  }else if(condition[num_test, 5] == "medium"){
...
box_omnibus <- function(row_index, cate_final, x_title){
  ...
  colnames(omni_result) <- c("omni_eq0", "omni_eq1", "omni_eq2", "omni_eq3", "category") #the names are in line with the names generated by categorize_mahalanobis() 
  for(i in 1:length(row_index)){
    if(row_index[i]){
      omni_result <- rbind(omni_result, cate_final[[i]][, c(5, 6, 7, 8, 33)])
    }
  }
  
  omni_result <- omni_result[-1, ]
  colnames(omni_result)[1:4] <- c("Omnibus+Eq(5)", "Omnibus+Eq(11)", "Omnibus+Eq(14)", "Omnibus+Eq(15)")
